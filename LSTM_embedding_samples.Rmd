...
```{r training and testing data read, include = TRUE}
...
```{r dynamic features function}
# Parámetros para definir el modelo:

dynamic_features = 1 # Variables dinámicas (RESULTADO, Wavelets)
static_features = 4  # Variables estáticas (EDAD_BIN, PACIENTE_SEXO,                                                 ANALIZADOR,CODIGO_PRUEBA)
...
```{r}
set.seed(123)
runs <- tuning_run("LSTM_samples_script.R",
                   runs_dir = file.path(resultsdir, "1_units_filter"),
                   sample = 0.1,
                   flags = list(
                     #lookback = c(50),
                     batch_size = c(256),
                     conv_filters = c(12, 24), 
                     lstm_units = c(8, 32)))
                     #dropout1 = c(0.3, 0.5),
...
forward_chain_cv <- function(s){
  ...
  time_points <- 
    do.call(quantile,
            list(as.POSIXct(as.Date(data_sel_wl[[1]])), q))
