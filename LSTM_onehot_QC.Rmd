...
val_code[[1]] %<>% arrange(., val_code[[1]]$TIEMPO_QC)

test_code <- code_cat_var(test_n)
test_code[[1]] %<>% arrange(., test_code[[1]]$TIEMPO_QC)
```
...
evaluation_results <- numeric(length(metric_names))
...
```{r}
test_select <- function(CLC, NIVEL, ANALIZADOR, FECHA) {
  ...
  x <- readline("Introduce el valor de x: ")
```


```{r model prediction, }

# Obtener las predicciones utilizando la función generadora de tests:

predictions <- predict(model, x = test_gen[[1]], steps = test_steps)

# Obtener las etiquetas verdaderas utilizando la función generadora
labels <- generator_labels

# Calcular la curva ROC y otras métricas
roc_obj <- roc(labels, predictions[, 2])
auc <- auc(roc_obj)
sensitivity <- sensitivity(roc_obj, levels = "specificity")
specificity <- specificity(roc_obj, levels = "sensitivity")
...
maes_naive <- evaluate_naive_method()
cbind(train_n$stdev[[3]], maes_naive*train_n$stdev[[3]])
