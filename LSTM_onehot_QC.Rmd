...
Las redes neuronales recurrentes requiren que los datos entren en forma de tensores con dimensión (n,k,p), siendo n el número de muestras, k la longitude una serie temporal y p el número de características o variables del modelo.
...
Se crea la función ```create_lstm_data.2```, análoga a ```create_lstm_data.1``` pero adaptada para leer como variable objetivo el siguiente valor al último presente en la serie temporal de longitud k.
...
## Métrica rmse.
Se define una función para calcular la métrica error cudrático medio (rmse) usando el backend de Keras y así poder usarla durante el entrenamiento y validación.
...
runs <- tuning_run("LSTM_QC_script.R",
                   runs_dir = file.path(resultsdir, "6_lb_units_regul"),
                   sample = 1,
                   flags = list(
                     lookback = c(50, 100),
                     batch_size = c(256),
...
b <- kable(runs)
runs[order(runs$eval, decreasing = TRUE), ]
```
...
model <- load_model_weights_hdf5(naive_model_final, filepath = file.path(resultsdir, "LSTM_QC_100.keras"))
...
matching_folders <- grep(".*cv\\.data.*", folders, value = TRUE)
...
# Evaluación por categorías.
 Con el objetivo de poder hacer evaluaciones selectivas del rendimiento, se crean códigos identificativos únicos de combinaciones de variables categóricas para el dataset de test, usando la función unique_lab_cases e indicando las posiciones de las variables categóricas en train_code$data que se deseen incluir. A modo de ejemplo se incluyen código de prueba y analizador:

```{r trainig set case grouping}
# Selección de variables categóricas del dataset de training (se puede hacer de cualquier dataset, pero se supone que si alguna categoría no está presente, no debería ocurrir en el de training)
categorical_vars <- c(3:34)
...
# Extraemos los códigos únicos del dataset de test:
test_indx <- unique_lab_cases(test_code$data, 
                             categorical_vars = categorical_vars)

# Usamos el diccionario de códigos creado anteriormente para asignar los índices correspondientes al dataframe de test:

test_indx$index <- code_dictionary$index[match(test_indx$code, code_dictionary$code)]
index_values <- unique(test_indx[,"index"])
test_indx$code <- NULL

# Creamos una función generadora selectiva por índices test_k_i:

test_k_i <- lapply(index_values, function(idx) {
  create_lstm_data.2(
...
# La añadimos a su correspondiente wrapper:
wrap_test_i <- function() {
  seq <- NULL
...
index_select <- function() {
  ...
  iqtest_opt <- iqtests[match(test_code$levels$CODIGO_PRUEBA,
                              iqtests$CODIGO_PRUEBA),]
  ...
  # Conversión de las variables introducidas en códigos:
  
  col_clc <- which(grepl(clc, names(test_indx)))-2
  col_level <- which(grepl(paste0("NIVEL_", level), names(test_indx)))-2
  col_anal <- which(grepl(anal, names(test_indx)))-2
  ...
  # Búsqueda en el diccionario el índice correspondiente al código generado:
...
# Ejecutamos index_select y creamos una función generadora con el índice elegido, añadiéndola a su wrapper:

index_pred <- index_select()

  test_pred <- lapply(index_pred, function(index_pred) {
    create_lstm_data.2(
      data = as.matrix(test_indx[test_indx[,"index"] == index_pred,
                                 -c(1, length(test_indx))]),
      lookback = lookback,
      delay = delay,
      batch_size = batch_size,
...
x <- list(test_pred[[1]]()[[1]][,,1, drop=F],
         test_pred[[1]]()[[1]][, lookback, -1])
