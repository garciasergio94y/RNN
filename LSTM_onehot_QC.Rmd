...
```{r load weights and export model}
set.seed(123)
# Evaluación del modelo:

# Ruta hacia model_0:
load_export <- function(model_name){
  # Crear una lista de archivos de resultados:
  files <- dir(resultsdir, recursive = TRUE,
                        full.names = TRUE)
  # Buscar por coincidencia los archivos .keras (archivos de weights)
  matching_files <- grep(".*\\.keras$", files, value = TRUE)
  
  # Cargar los weights a model_0 (modelo salvado cada vez que se ejecuta conditional_lstm():
  model_0 %>% load_model_weights_hdf5(filepath = matching_files)
  
  # Exportar modelo serializado a la carpeta resultados. Se deja la learning phase para evitar el error que ocurre en R al guardar modelos con una capa lambda no serializable. 
  export_savedmodel(model_0, file.path(resultsdir, model_name),
...
evaluation_result <- evaluate(model_0, wrap_test, steps = test_steps)
...
folders <- list.dirs(resultsdir, recursive = TRUE,
                        full.names = TRUE)

matching_folders <- grep(".*\\.cv$", folders, value = TRUE)

# Loop para obtener un informe de los modelos de cross validation:
cross_validation <- list()

for (folder in matching_folders) {
  model <- load_model_hdf5(folder,
                           custom_objects = list("rmse" = rmse),
                           compile = TRUE)
