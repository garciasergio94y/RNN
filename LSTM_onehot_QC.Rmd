...
batch_size <- 128 # Tamaño de batch de datos de la función generadora
...
### Función ```wrap_gen()```
Toma como argumento cualquier función generadora y devuelve los tensores mencionados.

```{r test wrapper}

wrap_gen <- function(generator) {
  seq <- NULL
...
conditional_lstm <- function(timesteps, rnn_units, vocabulary_size,
  ...
  save_model_hdf5(model, file.path(resultsdir, "naive_model")) 
  ...
  # Training
  history <- model %>%
    fit(wrap_gen(train_k),
        epochs = epochs,
        steps_per_epoch = train_steps,
        validation_data = val_gen,
        validation_steps = wrap_gen(val_k),
        callbacks = callbacks
        )
...
```{r}
conditional_lstm(number_sequence = 1)
```
...
runs <- tuning_run("LSTM_QC_script.R", 
                   runs_dir = file.path(resultsdir, "units&regul"),
                   sample = T,
                   flags = list(
                     units = c(8, 12, 16),
                     dropout1 = c(0.1, 0.4, 0.8),
                     recurrent_dropout1 = c(0.2, 0.4, 0.8),
...
files <- dir(resultsdir, recursive = TRUE,
                        full.names = TRUE)
matching_files <- grep(".*\\_0.keras$", files, value = TRUE)

# Carga del modelo:
model <- load_model_hdf5(matching_files,
