...
```{r training and testing data read, include=T}
...
## Selección de pruebas individuales o grupos de pruebas.

```{r test group selection}

qc_data_sel_wl %<>% subset(CODIGO_PRUEBA == "CLC00816" | 
                             CODIGO_PRUEBA == "CLC00817")
...
```{r training dataset subsetting and normalization, include=TRUE}
...
lookback <- 5    # Número de pasos de tiempo que se toman (k).
batch_size <- 32 # Tamaño de batch de datos de la función generadora
...
dense_u = 8 # Número de unidades de la capa densa.
...
```{r callbacks}
# Callbacks
  file_name <- paste0("lstm_QC_", 200, ".keras") 
...
runs <- tuning_run("LSTM_QC_script.R",
                   ...
                   flags = list(
                     ...
                     dense_u= c(8, 12),
                     dropout1 = c(0.1, 0.3),
                     recurrent_dropout1 = c(0.1, 0.3)
                     #dropout2 = c(0.1, 0.5),
                     #recurrent_dropout2 = c(0.1, 0.5),
                    # L2regularizer1 = c(0.01, 0.001),
                    # L2regularizer2 = c(0.01, 0.001)
...
compare_runs(c(file.path(
  resultsdir, "1_lb_units_regul/2023-06-30T20-47-23Z"), 
  file.path(
  resultsdir, "1_lb_units_regul/2023-06-30T20-23-52Z")))
```
...
matching_folders <- grep(".*cv202307[^/]*\\.keras", folders, value = TRUE)
...
for (folder in matching_folders) {
  ...
  sink(file.path(resultsdir, 
                 paste0("lstm_QC_cv",
                        format(Sys.time(), "%Y%m%d_%H%M%S"), ".html")))

cat("\nCross Validation Results:\n")

# Recorrer las listas de cross_validation
for (i in seq_along(cross_validation)) {
  list_name <- names(cross_validation)[i]
  
  cat("\nResults for:", list_name, "\n")
  
  table_html <- kable(cross_validation[[i]], format = "html") %>%
