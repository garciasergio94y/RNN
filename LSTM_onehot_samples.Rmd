...
```{r, k length time series generator function with wavelets}
...
create_lstm_data.3 <- 
  ...
  gen <- function() {
    ...
    for (j in 1:length(rows)) {
      indices <- (rows[[j]] - lookback + 1):(rows[[j]])
      samples[j,,,] <- array(data = c(data[indices, wl], 
                                data[indices, -c(wl, predseries)]),
                       dim = c(1, lookback / step, length(wl), dim(data[,-c(wl, predseries)])[[-1]]))
      targets[[j]] <- data[rows[[j]] + delay, predseries]
    }
    list(samples, targets)
  }
...
train_ss <- train_n$scaled_data %>% sub_sample(prop=0.1)
...
Se crean funciones generadoras que crearán series temporales de datos de longitud fija k (lookback) mediante la función create_lstm_data.1
...
val_k <-  create_lstm_data.1(
    ...
    batch_size = batch_size,
    min_index = min_index,
    max_index = nrow(val_data) - delay -1,
    predseries = ncol(val_data))

val_k <- create_lstm_data.3(
    data = val_data,
    lookback = lookback,
    delay = delay,
    batch_size = batch_size,
...
```{r model definition}
set.seed(123)
lapply(list(train_k()[[1]], val_k()[[1]], test_k()[[1]]), dim)
t <- train_k()
nkp <- dim(t[[1]])
nkwp <- dim(t[[1]])
...
model_1 %<>% compile(
  ...
  loss = loss_sigmoid_focal_crossentropy(alpha = 0.25, 
                                         gamma = 4,
