...
## Downsampling de casos con CTRL=0

Se emplea la función ```sub_sample``` diseñada para eliminar la proporción deseada de casos pertenecientes a la clase CTRL = 0 por muestre estratificado aleatorio.
```{r}
set.seed(123)

train_ss <- train_n$scaled_data %>% sub_sample(prop=0.3)

val_ss <- val_n %>% sub_sample(prop=0.3)

test_ss <- test_n %>% sub_sample(prop=0.3)

table(train_ss$NOMBRE_PRUEBA.x, train_ss$CTRL)

table(val_ss$NOMBRE_PRUEBA.x, val_ss$CTRL)

table(test_ss$NOMBRE_PRUEBA.x, test_ss$CTRL)
...
val_sel <- val_ss %>% subset(., !(CODIGO_PRUEBA %in% clc_other))
test_sel <- test_ss %>% subset(., !(CODIGO_PRUEBA %in% clc_other))
...
cat("Técnicas que no pueden entrenarse con estos datasets: ", clc_other)
```
...
Se crean funciones generadoras que crearán series temporales de datos de longitud fija k. Para contrarrestar el gran desbalanceo que existe en los datos entre series controladas y fuera de control, ya se crearon funciones generadoras globales, que se encargarán de ejecutar las funciones generadoras. Las funciones generadoras preseleccionan conjuntos de datos con valores definidos de categorías de la variable categórica que se desee, pudiendo estudiar el rendimiento del modelo en estos grupos.

### Dataset de training.

```{r trainig set case grouping}
...
categorical_vars <- c(10:38) # Todas las técnicas y analizadores
                             # sin wavelets
# categorical_vars <- c(12:38) # Técnicas seleccionadas, 
                              # sin analizador ni wavelets
...
# Downsampling por codificación.
num_date <- as.numeric(as.Date(train_code$data[,1]))
train_indx <- unique_lab_cases(train_code$data,
                               categorical_vars)
train_indx$code <- paste0(train_indx$CTRL, "#", train_indx$code, "#",
                          num_date
                          )
# Extraemos los códigos para realizar un diccionario de traducción de códigos únicos a índices numéricos:
code_values <- data.frame(unique(train_indx[,"code"]))
colnames(code_values) <- "code"
                          
code_values %<>%
  mutate(.,index = as.numeric(substr(code_values[,1], 1,1)))
code_values %<>% arrange(index)
code_subsample <- sample()


code_values %>% arrange(code_values$...2)
...
```{r trainig set k length time series creation}
...
lookback <- 50 # Longitud de la secuencia previa al target.
...
min_index <- 0 # Indice de partida de la serie temporal.
...
train_k <- mapply(function(ctrl, index) {#, date) {
  data <- as.matrix(
...
### Función generadora de series temporales.
...
```{r Validation set case grouping}
...
```{r Validation set k length time series creation}
...
```{r trainig and val set sampling probability function}
...
train_probs <- sampling_prob(combinations_train$index, 0.1)
val_probs <- sampling_prob(combinations_val$index, 1)

```

## Funciones generadoras globales ```train_gen``` y ```val_gen```.

Funciones generadoras globales de series temporales para los datasets de train y validación. Genera de manera indefinida series con los datos, bien de entrenamiento, bien de  validación. Se basa en un muestreo aleatorio de la lista de índices únicos con la que se han definidos las funciones generadoras almacenadas en train_k o val_k, que son las que realmente producen la serie temporal con el tamaño definido por lookbak y en cantidad definida por batch_size. Al haber incluido la variable resultado CTRL en la definición de los índices, y haber creado un vector de probabilidades dando mayor peso a los índices que representan casos con la variable CTRL = 1, la función de generación global llamará  con mayor probabilidad a las series de datos con valores 1 de la variable CTRL, resultando en un submuestreo de la categoría mayoritaria para compensar así el desequilibrio entre ambas.
Si la función global llama a una función generadora que no tenga suficientes datos como para generar una serie de longitud lookback, se genera una error que es capturado con trycatch y se intenta una siguiente llamada a otra generadora hasta que se devuelve alguna serie, estando diseñadas para funcionar indefinidamente.
...
test_gen <- function() {
  indices <- sample(length(test_pred))
...
# La creación de series temporales para el dataset de test tiene otro objetivo distinto al de los datasets de training y validación. Aquí interesa generar series por categorías seleccionadas de analizardor y código de prueba, para poder evaluar específicamente el comportamiento del modelo en cada caso. Por esto se crea un diccionario distinto al usado para los datasets de training y validación.

# Creación de los códigos identificativos únicos de combinaciones de variables categóricas para el set de test:

#categorical_vars_test <- c(12:38) # Todas las técnicas, 
                             #sin analizador ni wavelets
#categorical_vars_test <- c(10:38) # Todas las técnicas y analizadores
                             # sin wavelets
# categorical_vars <- c(12:30) # Técnicas seleccionadas, 
                              # sin analizador ni wavelets
#categorical_vars_test <- c(28:56) # Todas las técnicas, con                                                # analizadores, con wavelets 
#categorical_vars_test <- c(30:56) # Todas las técnicas, con                                                # analizadores, con wavelets 

test_indx <- unique_lab_cases(test_code$data,
                               categorical_vars = categorical_vars_test)

# Extraemos los códigos para realizar un diccionario de traducción de códigos únicos a índices numéricos:
code_values_test <- unique(test_indx[,"code"])
code_dictionary_test <- data.frame(code = code_values_test)
code_dictionary_test %<>%  
  mutate(., index = as.numeric(factor(code_values_test, 
                                      levels = code_values_test)))

# Sustituimos los códigos de train_indx por sus índices numéricos y ordenamos por fechas:
test_indx$index <- code_dictionary_test$index[match(test_indx$code, code_dictionary_test$code)]
test_indx$code <- NULL
test_indx %<>% arrange(., test_indx$TIEMPO_MUESTRA)

# Creación de lista de funciones generadoras de las series temporales de longitud k por categoría del dataset de test. Se generan series solo filtradas por las variables categóricas seleccionadas, sin tener por qué pertenecer al mísmo día de observación:

test_k <- mapply(function(idx) {
  data <- as.matrix(
  train_indx[train_indx[,"index"] == idx,
             - c(1, length(train_indx))])
...
idx = code_dictionary_test$index,
SIMPLIFY = T
) 

# Cálculo del número de steps de training:
test_steps <- (nrow(test_code$data) - lookback) / batch_size
```
...
# tensorboard dev upload --logdir ./ --name "LSTM_onehot_samples_2" --description "LSTM16do0.5rdo0.5 lr0.0001 ds0.1 cv1238 lb20"
...
sink(file.path(resultsdir, "GC_LSTM_onehot_samples_16.txt"))
...
index_select <- function(n) {
  ...
  names_opt <- iqtests$data.NOMBRE_PRUEBA.x[
    iqtests$data.CODIGO_PRUEBA %in% clc_opt]
  ...
  # Introducción de prueba y analizador:
  test_name <- names_opt[menu(names_opt, 
                      title = "Nombre de la prueba: ",
                      graphics = T)]
  
  clc <- iqtests$data.CODIGO_PRUEBA[
    iqtests$data.NOMBRE_PRUEBA.x==test_name]
  
  #anal <- anal_opt[menu(anal_opt, 
   #                     title = "Analizador: ",
    #                    graphics = T)]
    
  cat("Indice correspondiente a la prueba: ", clc, test_name, "\n")
  #,"analizador: ", anal, "\n")
  ...
  code <- rep(0, length(categorical_vars_test))
  ...
  index <- code_dictionary_test$index[match(code,
                                       code_dictionary_test$code)]
  print(index)
  return(list(index, test_name, clc))#, anal))
  
}

index_pred <- index_select(n)

  test_pred <- lapply(index_pred[[1]], function(idx) {
    create_lstm_data.1(
...
plot(y_true, y_pred, pch = 16, col = "blue", xlab = "y_true", ylab = "y_pred", 
     main = cat("Gráfico de dispersión: y_true vs. y_pred",
                index_pred[[2]]))

# Gráficos de evaluación del modelo de clasificación binaria: 
preds <- prediction(as.vector(y_pred), as.vector(y_true))
perf <- performance(preds, "sens", "f")
plot(perf,
     colorize=TRUE,
     main = cat("Sensivility vs F1 score", index_pred[[2]]))
abline(a=1, b= 0)
...
plot(perf,
     ...
     main = cat("Average TPR vs FNR", index_pred[[2]]))
...
plot(perf,
     ...
     colorize=TRUE,
     lwd= 3,
     main= cat("Precision vs Recall", index_pred[[2]]))
plot(perf,
     lty=3,
     col="grey78",
     add=TRUE)
...
plot(perf,
     ...
     colorize=TRUE,
     lwd= 3,
     main= cat("Sensitivity vs Specificity", index_pred[[2]]))
plot(perf,
     lty=3,
     col="grey78",
     add=TRUE)
...
plot(perf,
     ...
     colorize=TRUE,
     lwd= 3,
     main= cat("ROC curve", index_pred[[2]]))
plot(perf,
     lty=3,
     col="grey78",
     add=TRUE)
...
reverse_index_select <- function(index, n) {
  # Buscar el código correspondiente al índice
  code <- code_dictionary_test$code[match(index, code_dictionary_test$index)]
  ...
  anal <- names(test_indx)[n + pos_anal]
  clc <- names(test_indx)[n + pos_clc]
