...
```{r, concatenate, lstm and output layers, eval=FALSE}
...
```{r, model definition and compilation. Callbacks, eval=FALSE}
# Crear modelo
...
accuracy_train <- evaluate_naive_method(train_code$data, "CTRL")
accuracy_test <- evaluate_naive_method(val_code$data, "CTRL")

sprintf("Training accuracy: %.2f", accuracy_train)
sprintf("Validation accuracy: %.2f", accuracy_test)
